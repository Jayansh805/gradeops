...
Backends:
  ...
  - "gcs"    : Google Cloud Storage for files + Firestore for records
...
BackendType = Literal["local", "s3", "gcs"]
...
class BaseStorage(ABC):
    ...
    @abstractmethod
    def update_grade(self, grade_id: str, updates: dict) -> None:
...
class S3Storage(BaseStorage):
    ...
    def load_grades(
        ...
        resp = self.dynamo.query(
            KeyConditionExpression=Key("pk").eq(f"GRADE#{exam_id}")
        )
    ...
    def update_grade(self, grade_id: str, updates: dict) -> None:
        # Requires knowing exam_id for pk — pass it in updates as "_exam_id"
        exam_id = updates.pop("_exam_id", None)
        if not exam_id:
            raise ValueError("_exam_id must be included in updates for S3Storage.update_grade")
    ...
    def load_ocr(self, exam_id: str, student_id: str | None = None) -> list[OCRResult]:
        ...
        resp  = self.dynamo.query(KeyConditionExpression=Key("pk").eq(f"OCR#{exam_id}"))
        items = resp.get("Items", [])
        if student_id:
            items = [i for i in items if i.get("student_id") == student_id]
...
class StorageBackend:
    ...
    def create(backend: BackendType, **kwargs) -> BaseStorage:
        ...
        if backend == "local":
            ...
            raise NotImplementedError("GCS backend coming soon")
